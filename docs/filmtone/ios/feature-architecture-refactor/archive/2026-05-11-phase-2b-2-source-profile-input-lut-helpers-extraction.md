...
- Commit `c3a87601 refactor(ios): extract export source-profile and
  input-LUT helpers` landed Phase 2B-2 in the feature worktree.
- `FilmtoneExportSession.swift` reduced from 4488 → 4178 lines (−310).
  Two new files added under `Export/Internal/`:
  `ExportInputLutBuilder.swift` (input-LUT factories + Apple Log math +
  `synthesizedInputLutCache` `NSCache`) and
  `ExportSourceProfileResolver.swift` (sidecar provenance + `implTag`).
- All 4 call sites in `FilmtoneExportSession.swift` were rewritten from
  `Self.<helper>(...)` to `ExportInputLutBuilder.<helper>(...)` /
  `ExportSourceProfileResolver.<helper>(...)` and the
  `makePreparedLut(...) ?? makeActiveInputLut(...)` fallback at the
  init-time prepared-LUT site is preserved verbatim.
- Both helper types are `enum` namespaces (no instance state), matching
  the `feedback_no_extension_only_file_for_god_object_split` rule — no
  `extension FilmtoneExportSession` files were created.
- pbxproj 4-section registration verified for both files
  (`grep -c 'ExportInputLutBuilder' project.pbxproj` = 4,
   `grep -c 'ExportSourceProfileResolver' project.pbxproj` = 4).
  App target `PBXSourcesBuildPhase` file count changed by exactly +2.
- 5 cosmetic comment updates landed in
  `Editor/FilmtoneEditorFacade.swift:152`,
  `Editor/FilmtoneEditorStore.swift:1305`,
  `Source/FilmtoneSourceProfileCatalog.swift:12`,
  `Source/FilmtoneSourceProfileMath.swift:481`, and
  `Source/FilmtoneSourceProfileSchema.swift:105`.
- `bun run verify:ios` PASS (exit 0); `git diff --check` PASS; the
  pre-existing Swift 6 Sendable / actor-isolation warnings on
  `FilmtoneMediaRuntime`, `FilmtoneExportSession`, and
  `BenchmarkCollector` are unchanged.
- Next sub-stage candidate (per the Extraction Risk Rank in 2B-1
  archive): **Phase 2B-3 — `DepthPayloadManager`** extraction
  (`resolveVideoDepthReader` line 2794, enum `VideoDepthFramePullResult`
  line 2845, `pullNextVideoDepthFrame` line 2851; total ~70 lines).
  No cross-file readers today, uses local `DispatchSemaphore` only,
  `resolveVideoDepthReader` reads `request.depthEnabled` (parameterise
  on extraction). New `active.md` will pick this up.
