...
- [x] Created `Export/Internal/GradeRenderPipeline.swift` with the import
  set accepted by the build (`CoreGraphics` for `CGColorSpace`,
  `CoreImage` for kernels/`CIImage`/`CIVector`, `FilmLabSwiftCore` for
  `Phase0ParamsDTO`, `Foundation` retained with no behavioral impact).
- [x] Moved `preparedInputLut` / `preparedCreativeLut` ownership into
  `GradeRenderPipeline`; ExportSession now constructs both as init-local
  `let`s and passes them into `GradeRenderPipeline(...)`.
- [x] Kept `outputColorSpace` on `FilmtoneExportSession` and passed it
  into `GradeRenderPipeline` at init.
- [x] Moved the six methods (`applyInputLutStage`, `applyBaseGradeStage`,
  `applyToneCompressionStage`, `applyCreativeLutStage`, `applyPrintStage`,
  `applyLut`) into `GradeRenderPipeline`. `applyGrainStage` remained on
  ExportSession because it depends on `sourceSeed` and `timeSeconds`.
- [x] Rewired `applyGrade` color stage calls to
  `gradeRenderPipeline.applyInputLutStage` /
  `applyBaseGradeStage` / `applyToneCompressionStage` /
  `applyCreativeLutStage` / `applyPrintStage`. Stage order and the
  interleaved optics-compositor / grain calls are unchanged.
- [x] Rewired `applyRecordingMonitorGrade` to call the same five color
  stages on `gradeRenderPipeline`.
- [x] Registered `GradeRenderPipeline.swift` in pbxproj 4 sections with
  PBXBuildFile ID `B1E10001000000000000B20B` and PBXFileReference ID
  `B1E10001000000000000B20C` (sequenced after OpticsCompositor B209/B20A).
- [x] Moved-method grep on ExportSession returns 0 hits
  (`grep -nE "private func (applyInputLutStage|applyBaseGradeStage|applyToneCompressionStage|applyCreativeLutStage|applyPrintStage|applyLut)\b"`
  empty).
- [x] Prepared LUT grep on ExportSession returns only the 4 expected
  references at init: two `let preparedInputLut = …` /
  `let preparedCreativeLut = …` locals and the two `GradeRenderPipeline`
  init arguments. No stored properties.
- [x] `grep -c 'GradeRenderPipeline.swift' apps/capacitor-film-lab-ios/ios/App/App.xcodeproj/project.pbxproj`
  = 4.
...
- `apps/capacitor-film-lab-ios/ios/App/App/Export/FilmtoneExportSession.swift`
  2400 → 2262 lines (−138). The 6 moved methods accounted for −144 raw
  lines (input LUT 7 + base grade 62 + tone compression 24 + creative LUT 7
  + print 24 + LUT helper 20); ExportSession's new
  `gradeRenderPipeline` collaborator comment + field cost +6 lines, init
  prepared-LUT setup churn was 0 net.
- `apps/capacitor-film-lab-ios/ios/App/App/Export/Internal/GradeRenderPipeline.swift`
  new, 175 lines.
- `apps/capacitor-film-lab-ios/ios/App/App.xcodeproj/project.pbxproj`
  +4 lines (4-section registration; IDs B1E10001000000000000B20B /
  B1E10001000000000000B20C).
