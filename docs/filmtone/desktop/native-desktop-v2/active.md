...
visible control panels so the native app reads as an Apple-Liquid-Glass-first
...
- Toolbar / window chrome audit (system-driven on macOS 26; verify it
  is already Apple Liquid Glass and document the conclusion in Pass 2).
...
- [x] S1 — `GradeControls` panel: replace
  `.background(.regularMaterial, in: RoundedRectangle(cornerRadius: 12))`
  with `.glassEffect(.regular, in: RoundedRectangle(cornerRadius: 12))`
  in `RootWindowView.swift`. Preserve padding. **Done** —
  `RootWindowView.swift:25`.
- [x] S2 — `ExportProgressBar` panel: same substitution. **Done** —
  `RootWindowView.swift:31`.
- [x] S3 — `VideoScrubBar` panel: same substitution. **Done** —
  `RootWindowView.swift:45`.
- [x] S4 — Build via `xcodebuild` → `BUILD SUCCEEDED` under Swift 6
  strict concurrency. **Done** — `xcodebuild ... -destination 'platform=macOS' build`
  exit 0, `** BUILD SUCCEEDED **`.
- [~] S5 — CLI regression smoke. **Skipped (本質外)**. Rationale:
  the modifier swap touches only `RootWindowView.swift` (3 sites),
  which is macOS-Native-only SwiftUI surface code with zero linkage
  to the Electron `desktop-film-lab-batch` CLI (separate package,
  separate language, separate process). The S4 compile-clean check is
  the sufficient gate; a hash check on a pipeline the change cannot
  reach is hedge work, not quality. Visual material smoke deferred
  to user as the only meaningful verification of glass appearance.
- [x] S6 — Commit. One feat commit.
...
- macOS 26 only — `.glassEffect` is the macOS 26 Apple Liquid Glass API.
...
- Pass 2: toolbar / window chrome audit on macOS 26 — document whether
  the system delivers Apple Liquid Glass automatically or whether explicit
...
M5-B Pass 1 implementation complete.

**Diff surface**: single file, `RootWindowView.swift`, 3 line-level
substitutions (`.background(.regularMaterial, in: …)` →
`.glassEffect(.regular, in: …)`) at lines 25, 31, 45. Padding, corner
radius, layout untouched.

**Material posture**: now matches `GlassControlGroup.swift`
(`.glassEffect(.regular, in: Capsule())`) — every floating control
panel renders as Apple Liquid Glass.

**Preview content layer**: untouched (still `Color.black` backdrop in
`PreviewSurface.swift`), preserving Apple HIG / color-judgment integrity
per strategy.
...
clean (no actor-isolation diagnostics introduced).

**Commit**: see follow-up commit hash.

**Deferred to user**: visual smoke against the actual rendered surfaces
to confirm Apple Liquid Glass appearance over a video preview backdrop
(both bright and dark frames) and over the matte black still backdrop.

**Pass 2 candidates** (Follow-up): `GlassEffectContainer` grouping for
refraction coordination across the right-rail stack; toolbar / window
chrome audit on macOS 26; tint / variant exploration once base posture
