#!/usr/bin/env bash
# Source this file from release entrypoints. It loads app-local release env
# files while keeping explicitly exported shell values higher priority.
...
export FILMTONE_IOS_APP_ROOT="$(cd "${filmtone_release_script_dir}/.." && pwd)"

filmtone_release_env_files=(
  "${FILMTONE_IOS_APP_ROOT}/.env"
  "${FILMTONE_IOS_APP_ROOT}/.env.local"
  "${FILMTONE_IOS_APP_ROOT}/fastlane/.env"
  "${FILMTONE_IOS_APP_ROOT}/fastlane/.env.local"
)
...
if [ -n "${FILMTONE_RELEASE_ENV_FILE:-}" ]; then
  if [[ "${FILMTONE_RELEASE_ENV_FILE}" = /* ]]; then
    ...
    filmtone_release_env_files+=("${FILMTONE_IOS_APP_ROOT}/${FILMTONE_RELEASE_ENV_FILE}")
  fi
fi
