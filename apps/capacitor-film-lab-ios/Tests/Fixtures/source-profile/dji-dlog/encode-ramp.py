#!/usr/bin/env python3
"""Generate the DJI D-Log -> Rec.709 accuracy fixture."""
...
DLOG_CUT = 0.0078
DLOG_LINEAR_SLOPE = 6.025
DLOG_LINEAR_OFFSET = 0.0929
DLOG_LOG_A = 0.9892
DLOG_LOG_B = 0.0108
DLOG_LOG_C = 0.256663
DLOG_LOG_D = 0.584555

DGAMUT_TO_REC709 = np.array(
    [
...
def dlog_decode(v: np.ndarray) -> np.ndarray:
    return np.where(
        v <= 0.14,
        (v - DLOG_LINEAR_OFFSET) / DLOG_LINEAR_SLOPE,
        (np.power(10.0, 3.89616 * v - 2.27752) - DLOG_LOG_B) / DLOG_LOG_A,
    )


def dlog_encode(linear: np.ndarray) -> np.ndarray:
    return np.where(
        linear <= DLOG_CUT,
        DLOG_LINEAR_SLOPE * linear + DLOG_LINEAR_OFFSET,
        np.log10(linear * DLOG_LOG_A + DLOG_LOG_B) * DLOG_LOG_C + DLOG_LOG_D,
    )
...
def dlog_pixel_to_rec709(rgb_encoded: np.ndarray) -> np.ndarray:
    linear = dlog_decode(rgb_encoded)
    mapped = linear @ DGAMUT_TO_REC709.T
...
def emit_macbeth_patches(out_path: Path) -> None:
    rec709_to_dgamut = np.linalg.inv(DGAMUT_TO_REC709)
    dgamut_linear = MACBETH_REC709_LINEAR @ rec709_to_dgamut.T
    dlog_encoded = dlog_encode(np.clip(dgamut_linear, 0.0, None))
    rec709_encoded_expected = dlog_pixel_to_rec709(dlog_encoded)
    patches = []
    for i in range(MACBETH_REC709_LINEAR.shape[0]):
        patches.append(
            {
                "index": i,
                "dlogEncoded": [float(x) for x in dlog_encoded[i]],
...
def render_visualizations(out_dir: Path) -> None:
    ...
    render_strip(dlog_pixel_to_rec709(grayscale_rgb), out_dir / "expected-rec709.png")
...
def emit_provenance(out_path: Path, generated_count: int) -> None:
    ...
    body = f"""# DJI D-Log fixture provenance
...
def main() -> int:
    ...
    print(f"==> generating D-Log fixture in {here}")
