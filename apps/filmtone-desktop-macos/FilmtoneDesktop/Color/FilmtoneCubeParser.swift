...
// Verbatim port of iOS `FilmtoneCubeParser.swift` with two adaptations:
//   - Error type is local `FilmtoneCubeParseError` (iOS uses
//     `FilmtoneMediaError` + `filmtoneLocalized*` strings — not present on
//     Desktop yet).
//   - Output type is local `ParsedCubeLut` (iOS uses cross-bridge
//     `ParsedCubeLutDTO`).
// Algorithm is byte-identical: header parse → triple parse → DOMAIN_MIN/MAX
// resampling via trilinear interpolation onto the unit cube. The bundled
// Stone / Urban cubes pin DOMAIN_MIN = 0 / DOMAIN_MAX = 1 so the resampling
// branch is a no-op for v1.4 Pack 01, but we keep it for forward
// compatibility with future packs.

enum FilmtoneCubeParseError: Error, LocalizedError {
    case missingSize
    case valueCountMismatch(actual: Int, expected: Int)

    var errorDescription: String? {
        switch self {
        case .missingSize:
            return "The .cube file is missing LUT_3D_SIZE."
        case .valueCountMismatch(let actual, let expected):
            return "The .cube file has \(actual) values, but \(expected) are required."
        }
    }
}
...
struct ParsedCubeLut {
    let title: String
    let size: Int
    /// Flattened RGB triples in the iOS canonical order (b-major). Length =
    /// size * size * size * 3.
    let data: [Double]
    /// Pinned at 1.0 for built-in Pack 01 entries; reserved for user-imported
    /// LUTs in a later milestone.
...
enum FilmtoneCubeParser {
    ...
    static func parse(text: String, defaultTitle: String) throws -> ParsedCubeLut {
        ...
        guard let size, size > 1 else {
            throw FilmtoneCubeParseError.missingSize
        }

        let expectedCount = size * size * size * 3
        guard values.count == expectedCount else {
            throw FilmtoneCubeParseError.valueCountMismatch(
                actual: values.count,
