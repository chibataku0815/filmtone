...
Video export now only shows essential output (frame count, duration, completion).
Detailed per-frame profiling is available via `VITE_FILM_LAB_VERBOSE_VIDEO_EXPORT=true`.
