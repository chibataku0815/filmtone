...
export class AutomationClient {
  ...
  constructor(options: AutomationClientOptions = {}) {
    ...
    const repoCliPath = resolve(this.repoRoot, "apps/filmtone-desktop-macos/build/automation/FilmtoneAutomationCLI");
    const appHelperPath = "/Applications/Filmtone.app/Contents/MacOS/FilmtoneAutomationCLI";
    const explicitCliPath = options.cliPath ?? process.env.FILMTONE_AUTOMATION_CLI;
    this.checkedCliPaths = [explicitCliPath, appHelperPath, repoCliPath].filter(Boolean) as string[];
    this.cliPath = explicitCliPath
      ?? (existsSync(appHelperPath) ? appHelperPath : repoCliPath);
    ...
    this.allowBuild = options.allowBuild
      ?? (options.skipBuild === undefined ? process.env.FILMTONE_AUTOMATION_ALLOW_BUILD === "1" : !options.skipBuild);
  ...
  ensureBuilt(): void {
    ...
    if (!this.allowBuild) {
      throw new Error([
        "Filmtone automation CLI is missing.",
        "Install a Filmtone build that includes the signed automation helper, or set FILMTONE_AUTOMATION_CLI to its absolute path.",
        "For local development only, run bun run build:filmtone-automation or set FILMTONE_AUTOMATION_ALLOW_BUILD=1.",
        `Checked paths: ${this.checkedCliPaths.join(", ")}.`,
      ].join(" "));
    }
  ...
  private env(includePlanSecret: boolean): NodeJS.ProcessEnv {
