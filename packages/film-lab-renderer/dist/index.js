...
// src/webgl/passes/bloom.ts
function renderBloom(renderer, deps) {
  const {
    mips,
...
// src/webgl/passes/halation.ts
function renderHalation(renderer, deps) {
  const {
    mips,
...
// src/webgl/passes/diffusion.ts
function renderDiffusion(renderer, deps) {
  const {
    mips,
    downsampleMaterial,
    upsampleMaterial,
  ...
  du.uTexelSize.value.set(
    1 / rtColorGraded.width,
    1 / rtColorGraded.height
  );
  postMesh.material = downsampleMaterial;
  renderer.setRenderTarget(mips[0]);
  renderer.render(postScene, postCamera);
  ...
  const prevAutoClear = renderer.autoClear;
  renderer.autoClear = false;
  const weights = computeMipWeights(0.7, mips.length);
  for (let i = mips.length - 2; i >= 0; i--) {
    const lowRes = mips[i + 1];
    const highRes = mips[i];
    const uu = upsampleMaterial.uniforms;
...
// src/webgl/passes/lightShafts.ts
function renderLightShafts(renderer, sourceTexture, target, deps) {
  const {
    shaftMaterial,
    shaftBlendMaterial,
    rtShaft,
  ...
  su.uLightOrigin.value.set(shaftOriginX, 1 - shaftOriginY);
  su.uDecay.value = 0.92 + shaftDecay * 0.075;
...
// src/webgl/passes/motionBlur.ts
function getActiveFrameCount(shutterAngle, ringSize) {
  return activeMotionBlurFramesForShutter(
    shutterAngle,
    ringSize
  );
}
function computeBlendWeights(shutterAngle, activeFrames, ringFilledFrames, ringSize, weightCurve) {
...
function renderMotionBlur(renderer, sourceTexture, target, deps) {
  const {
    ringSize,
    ringCopyMaterial,
    ringBlendMaterial,
    ...
    motionThreshold,
    ring,
    getBlackTexture: getBlackTexture2
  } = deps;
  ...
  cu.uPrevSlot.value = ringFilledFrames > 0 ? rtRingBuffer[prevSlotIdx].texture : getBlackTexture2();
  cu.uTrail.value = ringFilledFrames > 0 ? trailIntensity : 0;
  ...
  const black = getBlackTexture2();
  for (let i = 0; i < N; i++) {
    const slotIndex = (ringWriteIndex - 1 - i + N * 2) % N;
    const filled = i < ringFilledFrames;
    bu[`uFrame${i}`].value = filled ? rtRingBuffer[slotIndex].texture : black;
...
// src/webgl/passes/crossFilter.ts
import * as THREE3 from "three";
var CROSS_FILTER_SPACING_RADIUS_MAX_PX = 48;
var CROSS_FILTER_SPACING_RADIUS_EXTRA_MAX_PX = 24;
function renderCentralBloom(renderer, sourceTexture, sourceWidth, sourceHeight, deps) {
...
function resolveCrossFilterDebugSource(view, currentPeakTarget, peakTarget, deps) {
  const { rtCrossThreshold, rtCrossPeak, rtCrossStreak } = deps;
  switch (view) {
    case "threshold":
      return rtCrossThreshold ? { texture: rtCrossThreshold.texture, gain: 8, falseColor: true } : null;
    case "peak":
      return rtCrossPeak ? { texture: rtCrossPeak.texture, gain: 16, falseColor: true } : null;
    case "peakSpaced":
...
function renderCrossFilterDebug(renderer, target, currentPeakTarget, peakTarget, deps) {
...
function renderCrossFilter(renderer, sourceTexture, target, deps) {
  const {
  ...
  if (!crossFilterStreakMaterial || !crossFilterPeakSpacingMaterial || !crossFilterPeakSpacingMaxMaterial || !crossFilterBlendMaterial || !crossFilterPeakMaterial || !crossFilterTemporalMaterial || !rtCrossThreshold || !rtCrossPeak || !rtCrossPeakSpacingWork || !rtCrossPeakSpacingMax || !rtCrossPeakSpaced || rtCrossPeakHistory.length < 2 || rtCrossStreak.length === 0) return passThroughHistory();
  ...
  const effectiveThreshold = isHard ? 0.7 : crossFilterThreshold;
  const effectiveSizeLimit = isHard ? 1 : crossFilterSizeLimit;
  const effectiveRandomness = isHard ? 1 : crossFilterRandomness;
  ...
  pk.uTexelSize.value.set(1 / rtCrossThreshold.width, 1 / rtCrossThreshold.height);
  ...
  if (crossFilterMinSpacing >= 1e-3) {
    const spacingBoost = Math.min(1, Math.max(0, crossFilterMinSpacing - 1));
    ...
    smu.uTexelSize.value.set(1 / rtCrossPeak.width, 1 / rtCrossPeak.height);
    ...
    spu.uTexelSize.value.set(1 / rtCrossPeak.width, 1 / rtCrossPeak.height);
  ...
  if (temporalHoldActive) {
    ...
    tu.uPrev.value = crossFilterPeakHistoryFilledFrames > 0 ? rtCrossPeakHistory[prevIndex].texture : getBlackTexture2();
  ...
  if (renderCrossFilterDebug(renderer, target, currentPeakTarget, peakTarget, {
    crossFilterDebugMaterial,
    crossFilterDebugView,
    postMesh,
    postScene,
  ...
  bu.uCentralBloom.value = isHard && rtCentralBloomMips[0] ? rtCentralBloomMips[0].texture : black;
...
function getBlackTexture() {
  if (!_blackTexture) {
    _blackTexture = new THREE4.DataTexture(
...
function hexToVec3(hex) {
  const c = new THREE4.Color(hex);
  return new THREE4.Vector3(c.r, c.g, c.b);
}
var WebGLBackend = class _WebGLBackend {
  ...
  halationIntensity = 0;
  halationSpread = 15;
  halationColor = new THREE4.Vector3(0.91, 0.063, 0.125);
  ...
  constructor(options) {
    ...
    this.material = new THREE4.ShaderMaterial({
      glslVersion: THREE4.GLSL3,
      vertexShader: options.vertexShader,
      fragmentShader: options.fragmentShader,
      uniforms: {
        uTexture: { value: null },
        uResolution: {
          value: new THREE4.Vector2(options.width, options.height)
        },
        uImageResolution: { value: new THREE4.Vector2(1280, 720) },
        uTime: { value: 0 },
        uExposure: { value: 0 },
        uContrast: { value: 1 },
        uSaturation: { value: 1 },
        ...
        uTint: { value: 0 },
        uShadowTint: { value: new THREE4.Vector3(0, 0, 0) },
        uHighlightTint: { value: new THREE4.Vector3(0, 0, 0) },
        uRGBShift: { value: 0 },
    ...
    this.geometry = new THREE4.PlaneGeometry(2, 2);
    this.mesh = new THREE4.Mesh(this.geometry, this.material);
    this.postScene = new THREE4.Scene();
    this.postCamera = new THREE4.OrthographicCamera(-1, 1, 1, -1, 0, 1);
    this.postGeometry = new THREE4.PlaneGeometry(2, 2);
    this.postMesh = new THREE4.Mesh(this.postGeometry);
    this.postScene.add(this.postMesh);
    this.bloomPrefilterMaterial = new THREE4.ShaderMaterial({
      glslVersion: THREE4.GLSL3,
    ...
    this.halationPrefilterMaterial = new THREE4.ShaderMaterial({
      glslVersion: THREE4.GLSL3,
      vertexShader: filmlabVertexShader,
      fragmentShader: halationPrefilterFragmentShader,
      uniforms: {
        uSource: { value: null },
        uHalationColor: { value: new THREE4.Vector3(0.91, 0.063, 0.125) },
        uThreshold: { value: 0.6 },
        uKnee: { value: 0.3 },
        uFlipY: { value: 0 }
      }
    });
    this.detailSoftnessMaterial = new THREE4.ShaderMaterial({
      glslVersion: THREE4.GLSL3,
      vertexShader: filmlabVertexShader,
      fragmentShader: detailSoftnessFragmentShader,
      uniforms: {
        uSource: { value: null },
        uTexelSize: { value: new THREE4.Vector2() },
    ...
    this.downsampleMaterial = new THREE4.ShaderMaterial({
      glslVersion: THREE4.GLSL3,
      vertexShader: filmlabVertexShader,
    ...
    this.upsampleMaterial = new THREE4.ShaderMaterial({
      glslVersion: THREE4.GLSL3,
      vertexShader: filmlabVertexShader,
    ...
    this.compositeMaterial = new THREE4.ShaderMaterial({
      glslVersion: THREE4.GLSL3,
      vertexShader: filmlabVertexShader,
  ...
  ensureRenderTargets() {
    ...
    const w = this.width;
    const h = this.height;
    this.rtColorGraded = new THREE4.WebGLRenderTarget(w, h, RT_OPTIONS);
    this.rtDetailSoftened = new THREE4.WebGLRenderTarget(w, h, RT_OPTIONS);
    this.rtCompareComposite = new THREE4.WebGLRenderTarget(w, h, RT_OPTIONS);
    this.rtBloomMips = [];
    for (let i = 0; i < _WebGLBackend.BLOOM_MIP_LEVELS; i++) {
      ...
      this.rtBloomMips.push(new THREE4.WebGLRenderTarget(mw, mh, RT_OPTIONS));
    }
    this.rtHalationMips = [];
    for (let i = 0; i < _WebGLBackend.HALATION_MIP_LEVELS; i++) {
      const mw = Math.max(1, Math.floor(w / Math.pow(2, i + 1)));
      const mh = Math.max(1, Math.floor(h / Math.pow(2, i + 1)));
      this.rtHalationMips.push(new THREE4.WebGLRenderTarget(mw, mh, RT_OPTIONS));
    }
  }
  ...
  ensureDiffusionResources() {
    ...
    for (let i = 0; i < _WebGLBackend.DIFFUSION_MIP_LEVELS; i++) {
      const mw = Math.max(1, Math.floor(w / Math.pow(2, i + 1)));
      const mh = Math.max(1, Math.floor(h / Math.pow(2, i + 1)));
      this.rtDiffusionMips.push(new THREE4.WebGLRenderTarget(mw, mh, RT_OPTIONS));
    }
  }
  /**
  ...
  ensurePostCompositeRenderTargets() {
    ...
    const w = this.width;
    const h = this.height;
    this.rtPostComposite0 = new THREE4.WebGLRenderTarget(w, h, RT_OPTIONS);
    this.rtPostComposite1 = new THREE4.WebGLRenderTarget(w, h, RT_OPTIONS);
  }
  /**
  ...
  ensureMotionBlurResources() {
    ...
    this.ringCopyMaterial = new THREE4.ShaderMaterial({
      glslVersion: THREE4.GLSL3,
      vertexShader: filmlabVertexShader,
    ...
    this.ringBlendMaterial = new THREE4.ShaderMaterial({
      glslVersion: THREE4.GLSL3,
      vertexShader: filmlabVertexShader,
    ...
    for (let i = 0; i < _WebGLBackend.MOTION_BLUR_RING_SIZE; i++) {
      this.rtRingBuffer.push(
        new THREE4.WebGLRenderTarget(this.width, this.height, RT_OPTIONS)
      );
    }
  }
  ...
  ensureDustResources() {
    ...
    this.dustMaterial = new THREE4.ShaderMaterial({
      glslVersion: THREE4.GLSL3,
      vertexShader: filmlabVertexShader,
      ...
      uniforms: {
        ...
        uTime: { value: 0 },
        uResolution: { value: new THREE4.Vector2() },
        uFlipY: { value: 0 }
      }
    });
  }
  ...
  ensureShaftResources() {
    ...
    this.shaftMaterial = new THREE4.ShaderMaterial({
      glslVersion: THREE4.GLSL3,
      vertexShader: filmlabVertexShader,
      fragmentShader: lightshaftsFragmentShader,
      uniforms: {
        uSource: { value: null },
        uLightOrigin: { value: new THREE4.Vector2(0.5, 0.85) },
        uDecay: { value: 0.96 },
        uDensity: { value: 0.98 },
    ...
    this.shaftBlendMaterial = new THREE4.ShaderMaterial({
      glslVersion: THREE4.GLSL3,
      vertexShader: filmlabVertexShader,
    ...
    this.rtShaft = new THREE4.WebGLRenderTarget(qw, qh, RT_OPTIONS);
  }
  ensureCrossFilterResources() {
    if (this.crossFilterStreakMaterial) return;
    this.crossFilterStreakMaterial = new THREE4.ShaderMaterial({
      glslVersion: THREE4.GLSL3,
      vertexShader: filmlabVertexShader,
      fragmentShader: crossFilterStreakFragmentShader,
      uniforms: {
        uSource: { value: null },
        uDirection: { value: new THREE4.Vector2(1, 0) },
        uTexelSize: { value: new THREE4.Vector2() },
        uLength: { value: 0.5 },
        uChromatic: { value: 0 },
    ...
    this.crossFilterBlendMaterial = new THREE4.ShaderMaterial({
      glslVersion: THREE4.GLSL3,
      vertexShader: filmlabVertexShader,
    ...
    this.crossFilterPeakSpacingMaterial = new THREE4.ShaderMaterial({
      glslVersion: THREE4.GLSL3,
      vertexShader: filmlabVertexShader,
    ...
    this.crossFilterPeakSpacingMaxMaterial = new THREE4.ShaderMaterial({
      glslVersion: THREE4.GLSL3,
      vertexShader: filmlabVertexShader,
      ...
      uniforms: {
        uSource: { value: null },
        uTexelSize: { value: new THREE4.Vector2() },
        uAxis: { value: new THREE4.Vector2(1, 0) },
        uRadiusPx: { value: 0 },
    ...
    this.rtCrossThreshold = new THREE4.WebGLRenderTarget(hw, hh, RT_OPTIONS);
    this.rtCrossPeak = new THREE4.WebGLRenderTarget(hw, hh, RT_OPTIONS);
    this.rtCrossPeakSpacingWork = new THREE4.WebGLRenderTarget(hw, hh, RT_OPTIONS);
    this.rtCrossPeakSpacingMax = new THREE4.WebGLRenderTarget(hw, hh, RT_OPTIONS);
    this.rtCrossPeakSpaced = new THREE4.WebGLRenderTarget(hw, hh, RT_OPTIONS);
    this.rtCrossStreak = [];
    for (let i = 0; i < 4; i++) {
      this.rtCrossStreak.push(new THREE4.WebGLRenderTarget(hw, hh, RT_OPTIONS));
    }
    this.crossFilterPeakMaterial = new THREE4.ShaderMaterial({
      glslVersion: THREE4.GLSL3,
      vertexShader: filmlabVertexShader,
    ...
    this.crossFilterTemporalMaterial = new THREE4.ShaderMaterial({
      glslVersion: THREE4.GLSL3,
      vertexShader: filmlabVertexShader,
    ...
    this.crossFilterDebugMaterial = new THREE4.ShaderMaterial({
      glslVersion: THREE4.GLSL3,
      vertexShader: filmlabVertexShader,
    ...
    for (let i = 0; i < 2; i++) {
      this.rtCrossPeakHistory.push(new THREE4.WebGLRenderTarget(hw, hh, RT_OPTIONS));
    }
  }
  /**
  ...
  ensureCentralBloomResources() {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      this.rtCentralBloomMips.push(new THREE4.WebGLRenderTarget(mw, mh, RT_OPTIONS));
    }
  }
  ...
  renderMotionBlur(renderer, sourceTexture, target) {
    ...
    const result = renderMotionBlur(renderer, sourceTexture, target, {
      ringSize: _WebGLBackend.MOTION_BLUR_RING_SIZE,
  ...
  renderCrossFilter(renderer, sourceTexture, target) {
    ...
    const result = renderCrossFilter(renderer, sourceTexture, target, {
  ...
  getRenderTargetLumaStats(rt) {
    ...
    for (let i = 0; i < size; i += 4) {
      const r = THREE4.DataUtils.fromHalfFloat(this.histogramHalfBuffer[i]);
      const g = THREE4.DataUtils.fromHalfFloat(this.histogramHalfBuffer[i + 1]);
      const b = THREE4.DataUtils.fromHalfFloat(this.histogramHalfBuffer[i + 2]);
  ...
  createLUT3DTexture(data, size) {
    const texture = new THREE4.Data3DTexture(data, size, size, size);
    texture.format = THREE4.RGBAFormat;
    texture.type = THREE4.FloatType;
    texture.minFilter = THREE4.LinearFilter;
    texture.magFilter = THREE4.LinearFilter;
    texture.wrapS = THREE4.ClampToEdgeWrapping;
    texture.wrapT = THREE4.ClampToEdgeWrapping;
    texture.wrapR = THREE4.ClampToEdgeWrapping;
  ...
  getParams() {
    return {
      ...
      bloomSoftKnee: this.bloomSoftKnee,
      halationSoftKnee: this.halationSoftKnee,
      halationColor: `#${new THREE4.Color(this.halationColor.x, this.halationColor.y, this.halationColor.z).getHexString()}`,
  ...
  getHistogramPixels() {
    ...
    for (let i = 0; i < size; i++) {
      out[i] = THREE4.DataUtils.fromHalfFloat(half[i]);
    }
...
var Viewport = class _Viewport {
  ...
  static async create(canvas, opts = {}) {
    ...
    if (prefer === "webgpu") {
      ...
      const { WebGPUBackend } = await import("./WebGPUBackend-FRO2DW4S.js");
...
function textureFromDrawable(drawable, maxTextureSize) {
  ...
  const texture = new THREE5.Texture(scaled);
...
var MediaLoader = class {
  ...
  async loadVideo(file) {
    return new Promise((resolve, reject) => {
      ...
      video.onloadeddata = () => {
        const texture = new THREE5.VideoTexture(video);
        texture.colorSpace = THREE5.SRGBColorSpace;
