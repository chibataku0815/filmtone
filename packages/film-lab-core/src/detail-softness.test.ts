...
describe("deriveDetailSoftnessUniforms", () => {
  test("detailSoftness = 0 produces identity-neutral uniforms", () => {
    ...
    expect(u.kernelRadiusPx).toBeGreaterThanOrEqual(0.55);
    expect(u.kernelRadiusPx).toBeLessThanOrEqual(1.45);
  });
  ...
  test("detailSoftness above 0.34 clamps to effectiveMax", () => {
  ...
  test("sourceDetailBias sums into effective then re-clamps to [0, 0.34]", () => {
    expect(
      deriveDetailSoftnessUniforms(0.2, { sourceDetailBias: 0.3 })
  ...
  test("kernelRadiusPx stays inside [0.55, 1.45] across the input range", () => {
    for (const v of [0, 0.05, 0.18, 0.3, 0.34, 0.5, -0.1]) {
  ...
  test("kernelRadiusPx increases monotonically with effectiveDetailSoftness", () => {
    const samples = [0, 0.05, 0.1, 0.18, 0.24, 0.3, 0.34].map((v) =>
    ...
    expect(samples[0]!.kernelRadiusPx).toBeCloseTo(0.55, 10);
    expect(samples.at(-1)!.kernelRadiusPx).toBeCloseTo(1.45, 10);
  });
  ...
  test("constants required by 4-renderer parity stay stable", () => {
    ...
    expect(u.chromaAttenScale).toBe(0.85);
